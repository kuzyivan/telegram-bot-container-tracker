...
def upload_map_to_github(html_file_path):
    ...
    subprocess.run(["git", "-C", repo_dir, "config", "user.name", "kuzyivan"], check=True, env=env)
    subprocess.run(["git", "-C", repo_dir, "config", "user.email", "kuzyivan@example.com"], check=True, env=env)
